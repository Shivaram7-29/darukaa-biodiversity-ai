...
1. **Scientific Grounding & Numerical Precision**:
   ...
   - Adhere to the following authoritative scientific baselines from your knowledge base:
     * **Soil Organic Carbon (SOC) Thresholds**: SOC < 0.5% indicates critically low / severely degraded organic carbon (< 1.0% is low/degraded; 2.0% is moderate). When interpreting low values such as 0.3% SOC, state qualitatively that '0.3% SOC indicates very low (or critically low) soil organic carbon'; do NOT cite '2–6% typical healthy agricultural soils' unless that exact range and attribution are explicitly verified in the retrieved evidence. SOC accumulation occurs over 3–5 years; do NOT fabricate annual percentage accumulation figures (e.g. '+0.2%/yr'). Water retention gains from SOC are strictly texture-dependent (greatest in sandy soils; structural aggregation in clays)—never state unbacked universal volumetric figures (such as '+1.5–2.0% water capacity per 1% SOC').
     ...
     * **Soil Moisture & Water Management**: Distinguish raw soil moisture percentage from percentage of field capacity (% FC). Do NOT interpret an input such as 'soil moisture = 20%' as '20% field capacity' unless the user explicitly provided moisture as % of field capacity. Do NOT say 'stress begins below 30% field capacity' when the input is simply soil moisture percentage. Keep the evidence-supported qualitative conclusion that 20% soil moisture indicates water limitation/stress for most crops in this scenario, but do not convert it into field-capacity units. Drip irrigation achieves 30–50% water savings over flood irrigation, improving root-zone moisture and plant-available water (FAO Irrigation Papers). Do NOT claim that '50–75% field capacity is the optimal range', and never generate 50–75% FC targets, fixed moisture improvement percentages, fixed water-holding-capacity multipliers, or invented evaporation reductions. Always use qualitative wording such as 'improve root-zone moisture and plant-available water' unless an exact number is directly supported by retrieved evidence.
    ...
    def _parse_recommendations_from_response(text: str) -> tuple[list[dict], str | None]:
        ...
        for block in action_splits[1:]:
            ...
            meas_match = re.search(
                r'\*\*(?:Measurable|Expected|Anticipated)?\s*(?:Improvement|Estimate|Outcome|Quantitative\s+Estimate)(?:\s+Estimate)?(?:\s*\(.*?\))?\s*[:\-]?\*\*\s*[:\-]?\s*(.*?)(?=\n\s*[-*]\s*\*\*|\n\s*####?\s+|\Z)',
                block, re.S | re.I,
            )
